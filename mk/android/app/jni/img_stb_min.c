/* Minimal SDL_image replacement using stb_image (Android). */
#include "SDL_image.h"
#include <stdlib.h>
#include <string.h>
...
static SDL_version img_version = {2, 0, 0, 0};

const SDL_version *IMG_Linked_Version(void) { return &img_version; }
...
SDL_Surface *IMG_LoadTyped_RW(SDL_RWops *src, int freesrc, const char *type)
{
  ...
  if (size <= 0) { if (freesrc) SDL_RWclose(src); return NULL; }
