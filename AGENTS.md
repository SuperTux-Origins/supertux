...
**WebAssembly:** `nix/wasm.nix` builds offline SDL2 (+ SDL2_image) via `emcmake` (same flake `sdl2-src` / `sdl2-image-src` inputs as Android — **not** Emscripten’s network `-sUSE_SDL=2` port). Game flags: `ENABLE_SDL2=ON`, `ENABLE_GLES2=ON` (WebGL), `ENABLE_SOUND=OFF` on first bring-up. Offline static **zlib** is a separate flake output (`wasm-zlib-libs`), linked via `ZLIB_ROOT`. Nested title/session/worldmap loops currently rely on **ASYNCIFY**; `GameSession::frame()` and `WorldMap::frame()` are extracted as the first step toward a real frame pump (see `TODO.md` Phase 5). Assets preload from `data/` to `/data` when present. Config and savegames persist via **IDBFS** under `/home/web_user`.
