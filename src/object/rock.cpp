...
Rock::Rock(ReaderMapping const& reader) :
  ...
  on_ungrab_script()
{
  on_grab_script = reader.get("on-grab-script", std::string());
  on_ungrab_script = reader.get("on-ungrab-script", std::string());
  SoundManager::current()->preload(ROCK_SOUND);
  set_group(COLGROUP_MOVING_STATIC);
}
...
Rock::Rock(ReaderMapping const& reader, std::string const& spritename) :
  ...
  on_ungrab_script()
{
  if (!reader.read("on-grab-script", on_grab_script)) on_grab_script = "";
  if (!reader.read("on-ungrab-script", on_ungrab_script)) on_ungrab_script = "";
  SoundManager::current()->preload(ROCK_SOUND);
  set_group(COLGROUP_MOVING_STATIC);
}
